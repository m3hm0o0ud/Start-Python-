...
المتغيرات في لغة بايثون هي عناصر تُستخدم لتخزين البيانات التي يتم معالجتها أو استخدامها في البرنامج. يمكنك التفاعل مع المتغيرات عن طريق إسناد القيم إليها واسترجاع هذه القيم لاحقًا. سأشرح لك مفهوم المتغيرات بالتفصيل خطوة بخطوة:
...
#### 2. استخدام علامات الاقتباس المفردة داخل علامات اقتباس مزدوج
...
أفضل طريقة
 ...
 لإخراج المتغيرات المتعددة في دالة `print()` هي فصلها بفواصل، وهذا يدعم أنواع البيانات المختلفة:
```python
x = 5
y = "John"
...
بالطبع! إليك مجموعة من الأسئلة السهلة للواجب المنزلي في البرمجة بلغة بايثون:

### الواجب المنزلي السهل

#### 1. برنامج لعرض رسالة ترحيبية
اكتب برنامجًا يطلب من المستخدم إدخال اسمه، ثم يعرض رسالة ترحيبية تحتوي على الاسم المدخل.

#### 2. برنامج لجمع عددين
اكتب برنامجًا يطلب من المستخدم إدخال عددين، ثم يعرض ناتج جمع هذين العددين.

#### 3. برنامج لحساب عمر المستخدم
اكتب برنامجًا يطلب من المستخدم إدخال سنة ميلاده، ثم يحسب عمره الحالي ويعرضه.

#### 4. برنامج لحساب مربع عدد
اكتب برنامجًا يطلب من المستخدم إدخال عدد، ثم يعرض مربع هذا العدد.

#### 5. برنامج لتحويل الدقائق إلى ساعات ودقائق
اكتب برنامجًا يطلب من المستخدم إدخال عدد الدقائق، ثم يحولها إلى ساعات ودقائق ويعرض النتيجة.

#### 6. برنامج لتحويل درجات الحرارة من مئوية إلى فهرنهايت
اكتب برنامجًا يطلب من المستخدم إدخال درجة حرارة بالمئوية، ثم يحولها إلى فهرنهايت ويعرض النتيجة.

#### 7. برنامج للتحقق مما إذا كان العدد زوجيًا أم فرديًا
اكتب برنامجًا يطلب من المستخدم إدخال عدد، ثم يتحقق مما إذا كان هذا العدد زوجيًا أم فرديًا ويعرض النتيجة.

#### 8. برنامج لحساب طول النص
اكتب برنامجًا يطلب من المستخدم إدخال نص، ثم يعرض طول هذا النص (عدد الأحرف فيه).

#### 9. برنامج لتحويل الجنيهات إلى دولارات
اكتب برنامجًا يطلب من المستخدم إدخال مبلغ بالجنيهات، ثم يحوله إلى دولارات بناءً على سعر الصرف المحدد.

#### 10. برنامج لحساب مساحة مستطيل
اكتب برنامجًا يطلب من المستخدم إدخال طول وعرض مستطيل، ثم يحسب مساحته ويعرض النتيجة.
...
```python
name = input("ما اسمك؟ ")   # طلب إدخال اسم المستخدم

print("مرحبًا، " + name + "!")   # عرض رسالة ترحيبية
```

#### 2. برنامج لجمع عددين

```python
num1 = int(input("أدخل العدد الأول: "))   # طلب إدخال العدد الأول

num2 = int(input("أدخل العدد الثاني: "))    # طلب إدخال العدد الثاني

sum_result = num1 + num2   # حساب الناتج وعرضه
print("ناتج الجمع هو: " + str(sum_result))
```

#### 3. برنامج لحساب عمر المستخدم

```python
birth_year = int(input("أدخل سنة ميلادك: "))    # طلب إدخال سنة الميلاد

current_year = 2024    # حساب العمر
age = current_year - birth_year

print("عمرك هو: " + str(age) + " سنة")   # عرض العمر
```

#### 4. برنامج لحساب مربع عدد

```python
number = int(input("أدخل عددًا: "))    # طلب إدخال العدد

square = number * number    # حساب مربع العدد

print("مربع العدد هو: " + str(square))    # عرض النتيجة
```

#### 5. برنامج لتحويل الدقائق إلى ساعات ودقائق

```python
total_minutes = int(input("أدخل عدد الدقائق: "))    # طلب إدخال عدد الدقائق

hours = total_minutes // 60    # حساب الساعات والدقائق المتبقية
minutes = total_minutes % 60    # عرض النتيجة

print(f"{total_minutes} دقيقة تعادل {hours} ساعة و {minutes} دقيقة.")
```

### خاتمة

هذه الأسئلة تهدف إلى تعزيز فهمك للمفاهيم الأساسية في البرمجة بلغة بايثون. تأكد من تجربة كل سؤال وكتابة التعليقات المناسبة لتوضيح كل خطوة في البرامج الخاصة بك.
